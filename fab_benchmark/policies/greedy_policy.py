...
"""
Greedy policy (v1): move toward predicted highest concentration based on belief_summary.topk.

Baseline: exploitation-heavy.
"""
...
class GreedyPolicy(Policy):
    ...
    def reset(self, seed: int, scenario_spec: Dict[str, Any]) -> None:
        ...
        self._rng = random.Random(int(seed) + 771)
        ...
        self._last_action = {"type":"velocity","v":0.0,"w":0.0,"action_id":"greedy_init"}
    ...
    def step(self, t: float, pose: Dict[str, float], measurement: Dict[str, float], belief_summary: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        # use topk hypotheses as likely sources
        topk = []
        ...
        for ci in range(candidates):
            ...
            # expected concentration at candidate point under topk mixture
            s = 0.0
            for h in topk:
                pos = h["pos"]; p = float(h["p"])
                s += p * self._gas.query(tx,ty,1.0,t,self._theta,(pos[0],pos[1],pos[2]),self._q,self._start)
            if s > best_score:
                best_score = s
                best_target = (tx,ty)
                best = ci
        ...
        action = {"type":"velocity","v": float(v), "w": float(w), "action_id": f"greedy_cand_{best}"}
