...
def _room_of(x: float, y: float, bboxes: Dict[str, Tuple[float, float, float, float]]) -> Optional[str]:
    for rid, (x0, y0, x1, y1) in bboxes.items():
...
class GasModelA:
    ...
    def query(
        ...
        c_adv = q * acc * dtau * scale

        # ------------------------------------------------------------------
        # IMPORTANT (paper-friendly surrogate):
        # We want a physically-intuitive "near source is always highest" field:
        # - Still keep HVAC advection (so the plume stretches downwind),
        # - But prevent cases where downwind becomes higher than the source region.
        #
        # We do this with:
        # (1) distance falloff on the advected field, and
        # (2) an isotropic local component anchored at the source (no advection),
        # then take max().
        # ------------------------------------------------------------------
        dist = float(math.hypot(x - sx, y - sy))
        alpha = float(self.mode_params.get("source_falloff_alpha", 0.08))  # larger -> stronger near-source peak
        c_adv *= float(math.exp(-alpha * dist))

        c = c_adv

        if bool(self.mode_params.get("ensure_source_peak", True)):
            # local component (no advection): backtrace is identity
            acc_local = 0.0
            for tau in taus:
                dx0 = x - sx
                dy0 = y - sy
                sigma2_0 = self.source_radius ** 2 + 2.0 * self.D * float(tau)
                sigma2_0 = max(sigma2_0, 1e-6)
                norm_xy0 = 1.0 / (2.0 * math.pi * sigma2_0)
                g_xy0 = norm_xy0 * math.exp(-(dx0 * dx0 + dy0 * dy0) / (2.0 * sigma2_0))

                sigma_z2_0 = base_sigma_z2 + 2.0 * self.D_z * float(tau)
                sigma_z2_0 = max(sigma_z2_0, 1e-6)
                g_z0 = (1.0 / math.sqrt(2.0 * math.pi * sigma_z2_0)) * math.exp(-(dz * dz) / (2.0 * sigma_z2_0))

                acc_local += math.exp(-self.lambda_decay * float(tau)) * g_xy0 * g_z0

            scale_local = float(self.mode_params.get("scale_local", scale))
            c_local = q * acc_local * dtau * scale_local
            boost = float(self.mode_params.get("source_peak_boost", 1.0))
            c = max(c, boost * c_local)

        # Topology attenuation (optional)
        c *= self._topology_factor(x, y, sx, sy)
